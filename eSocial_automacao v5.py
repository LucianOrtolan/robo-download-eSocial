    ...
    if event == sg.WINDOW_CLOSED:
        ...
        if sistema_validacao.validar_chave(chave_usuario):         
            ...
            while True:
                        ...
                        for linha in sheet_empresas.iter_rows(min_row=int(valores['linhaini']), max_row=int(valores['linhafim'])):
                            ...
                            if documento >= 15:
                                ...
                                while (contador < 5):
                                    ...
                                    contador += 1
                                    print(f'Número de requisições: {contador}')                                    

                                    if (contador % 5 == 0 or end_date == data_corte):
                                        linha_celula = linha[4]

                                        if hasattr(linha_celula, 'row'):
                                            linha_atual = linha_celula.row
                                            sheet_empresas[f'F{linha_atual}'] = 'Solicitação realizada'
                                            sheet_empresas[f"G{linha_atual}"] = "Última data solicitada: " + end_date_string
                                            workbook.save(valores['caminho'])

                                print(f'Última data solicitada: {end_date}')
                                driver.find_element(By.XPATH, '//*[@id="header"]/div[2]/a').click()
                ...
                proxima_data = end_date + timedelta(days=1)
                print(proxima_data.strftime('%d/%m/%Y'))
                
                if proxima_data > end_date:
                    janela['dataini'].update('FIM')
                else:
                    janela['dataini'].update(proxima_data.strftime('%d/%m/%Y'))
